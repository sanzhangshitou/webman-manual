...
또한 keep-alive를 비활성화한 상태에서 스트레스 테스트를 수행하면, 클라이언트의 로컬 포트가 TIME_WAIT 상태의 연결로 인해 빠르게 소진됩니다. 이는 총 요청 수가 일정 수량(일반적으로 약 2.8만 개)을 초과하면 요청 실패가 발생하는 현상으로 나타납니다.
...
**환경**
알리바바 클라우드 4코어 4G, 로컬 MySQL 데이터베이스, 10만 개의 레코드에서 무작위로 1개를 조회하여 JSON으로 반환, 로컬 스트레스 테스트.
...
> **팁**
> ORM을 사용하면 성능이 감소하지만, 99%의 비즈니스 시나리오에서는 이미 성능이 충분 이상입니다. 나머지 1%에 해당하는 경우에도 CPU나 서버를 추가하여 쉽게 해결할 수 있습니다.
> 개발 효율성, 유지보수성, 성능 등 여러 지표 사이에서 균형을 찾아야 하며, 성능만을 맹목적으로 추구해서는 안 됩니다.
