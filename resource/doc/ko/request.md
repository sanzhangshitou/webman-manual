...
## 헬퍼 함수 input()
`$request->input()`과 유사하게 모든 매개변수를 가져올 수 있습니다. input() 헬퍼 함수에는 두 개의 매개변수가 있습니다:
1. name: 가져올 매개변수 이름 (비어 있으면 모든 매개변수의 배열을 가져옴)
2. default: 기본값 (첫 번째 매개변수로 가져오기에 실패한 경우 사용)

**예시**
```php
// 매개변수 name 가져오기
$name = input('name');
// 매개변수 name 가져오기, 없으면 기본값 사용
$name = input('name', '홍길동');
// 모든 매개변수 가져오기
$all_params = input();
```
...
## 모든 입력 가져오기
`post`와 `get`의 집합을 포함합니다.
```php
$request->all();
```

## 특정 입력 값 가져오기
`post`와 `get`의 집합에서 특정 값을 가져옵니다.
```php
$request->input('name', $default_value);
```

## 일부 입력 데이터 가져오기
`post`와 `get`의 집합에서 일부 데이터를 가져옵니다.
```php
...
```
자세한 내용은 [컨트롤러 매개변수 바인딩](controller.md#컨트롤러-매개변수-바인딩)을 참조하세요.

## 업로드 파일 가져오기

> **참고**
> 파일 업로드에는 `multipart/form-data` 형식의 폼이 필요합니다.
...
`$request->getRealIp()`는 HTTP 헤더의 `x-forwarded-for`, `x-real-ip`, `client-ip`, `x-client-ip`, `via` 필드에서 실제 IP를 가져오려고 시도합니다.

> HTTP 헤더는 쉽게 위조될 수 있기 때문에, 이 메서드로 얻은 클라이언트 IP는 100% 신뢰할 수 없습니다. 특히 `$safe_mode`가 false일 때입니다. 프록시를 통해 클라이언트의 실제 IP를 확인하는 더 신뢰할 수 있는 방법은, 안전하다고 알려진 프록시 서버 IP를 확인하고 실제 IP를 담고 있는 HTTP 헤더를 명확히 알고 있는 것입니다. `$request->getRemoteIp()`가 반환한 IP가 이미 알려진 안전한 프록시 서버로 확인되면, `$request->header('실제 IP를 담은 HTTP 헤더')`로 실제 IP를 가져올 수 있습니다.
...
## 매개변수 덮어쓰기
요청 매개변수를 필터링한 후 요청 객체에 다시 할당하고 싶을 때가 있습니다. 이때 `setGet()`, `setPost()`, `setHeader()` 메서드를 사용할 수 있습니다.

#### GET 매개변수 덮어쓰기
```php
$request->get(); // ['name' => 'tom', 'age' => 18]를 얻었다고 가정
$request->setGet(['name' => 'tom']);
$request->get(); // 최종적으로 ['name' => 'tom']을 얻음
```

> **참고**
> 예시와 같이 `setGet()`은 모든 GET 매개변수를 덮어씁니다. `setPost()`, `setHeader()`도 동일하게 동작합니다.
...
$request->post(); // 필터링된 post 매개변수 가져오기
```

#### HEADER 매개변수 덮어쓰기
```php
$request->setHeader(['host' => 'example.com']);
$request->header('host'); // 출력: example.com
```
