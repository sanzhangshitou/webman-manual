...
`#[Route]` hỗ trợ nhiều phương thức HTTP và tên route:
...
// Chỉ đường dẫn và phương thức, không có tên route
#[Route('/user/update', 'PUT')]
public function update() { ... }
```

### Tham số đường dẫn và regex

Đường dẫn route chú thích hỗ trợ cùng cú pháp tham số với `config/route.php`, bao gồm regex và tham số tùy chọn:

```php
#[Get('/user/{id:\d+}')]           // Chỉ id số
public function show($id) { ... }

#[Get('/user[/{name}]')]           // name tùy chọn: /user hoặc /user/xxx
public function list($name = null) { ... }

#[Any('/user/[{path:.+}]')]        // Khớp /user và /user/ với bất kỳ hậu tố nào
public function catchAll($path = null) { ... }
```

Xem mục 「Tham số của định tuyến」 trong tài liệu này.
...
// Sử dụng: route('user.show', ['id' => 123])  =>  /user/123
```
