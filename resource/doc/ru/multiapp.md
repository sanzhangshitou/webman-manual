...
В webman каждый каталог под app может рассматриваться как отдельное приложение.
...
└── admin
    ...
    └── view
```
Таким образом, когда вы обращаетесь по адресу `http://127.0.0.1:8787/{контроллер}/{метод}`, вы обращаетесь к контроллеру и методу в `app/controller`. Когда путь начинается с `api` или `admin`, вы обращаетесь к контроллеру и методу в соответствующем каталоге.
