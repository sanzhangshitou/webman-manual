...
> **Подсказка**
> Кросс-доменные запросы могут вызывать предварительный запрос OPTIONS. Мы не хотим, чтобы запрос OPTIONS попадал в контроллер, поэтому возвращаем пустой ответ (`response('')`) для перехвата запроса.
> Если вашему интерфейсу нужен маршрут, используйте `Route::any(..)` или `Route::add(['POST', 'OPTIONS'], ..)`.
...
## Промежуточное ПО контроллера и метода

С помощью аннотаций можно назначить промежуточное ПО определённому контроллеру или определённому методу контроллера.
...
```
## Передача параметров через конструктор промежуточного ПО

Конфигурационный файл поддерживает прямую инстанциацию промежуточного ПО, что позволяет удобно передавать параметры через конструктор.
...
Пример использования параметров в промежуточном ПО:
```
<?php
namespace app\middleware;
...
- Порядок выполнения: `глобальное промежуточное ПО` -> `промежуточное ПО приложения` -> `промежуточное ПО контроллера` -> `промежуточное ПО маршрута` -> `промежуточное ПО метода`.
- При наличии нескольких промежуточных слоёв на одном уровне они выполняются в порядке их фактической конфигурации.
- Запросы 404 по умолчанию не вызывают ни одного промежуточного ПО (однако можно добавить промежуточное ПО через `Route::fallback(function(){})->middleware()`).
...
## Получение информации о текущем маршруте в промежуточном ПО
...
В процессе обработки бизнес-логики может возникнуть исключение. В промежуточном ПО можно использовать `$response->exception()` для получения исключения.
...
## Суперглобальное промежуточное ПО
...
## Добавление промежуточного ПО в определённый плагин

Иногда мы хотим добавить промежуточное ПО для какого-то [приложения-плагина](app/app.md) без изменения кода плагина (поскольку обновление может привести к потере изменений), в этом случае мы можем настроить промежуточный слой в основном проекте для плагина.

В файле `config/middleware.php` настроим его следующим образом:
```php
return [
    'plugin.ai' => [], // Добавление промежуточного слоя для плагина ai
    'plugin.ai.admin' => [], // Добавление промежуточного ПО для модуля admin плагина ai (каталог plugin\ai\app\admin)
];
```
