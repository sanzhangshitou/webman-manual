...
Кроме того, при проведении нагрузочного тестирования без keep-alive локальные порты клиента быстро исчерпываются из-за соединений в состоянии TIME_WAIT. Это проявляется в виде неудачных запросов, когда общее количество запросов превышает определённый порог (обычно около 28 000).
...
**Среда**
Облачный сервер Alibaba 4 ядра 4 ГБ, локальная база данных MySQL, случайная выборка одной записи из 100 000 с возвратом в JSON, локальное нагрузочное тестирование.
...
> **Примечание**
> Несмотря на снижение производительности при использовании ORM, для 99 % бизнес-сценариев производительность уже более чем достаточна. Если вы относитесь к оставшемуся 1 %, вы можете легко решить это, добавив больше процессоров или серверов.
> Мы должны найти баланс между эффективностью разработки, сопровождаемостью и производительностью, а не просто стремиться к максимальной производительности.
