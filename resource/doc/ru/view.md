...
1. Если путь начинается с `/`, используйте этот путь напрямую для поиска файла представления.
2. Если путь не начинается с `/` и приложение не мультиприложением, используется соответствующий файл представления из `app/view/`.
3. Если путь не начинается с `/` и используется [мультиприложение](multiapp.md), используется соответствующий файл представления из `app/имя_приложения/view/`.
4. Если параметр шаблона не передан, файл шаблона автоматически ищется по правилам 2 и 3.
