...
webman支援靜態檔案存取，靜態檔案都放置於`public`目錄下。例如存取`http://127.0.0.8787/upload/avatar.png`實際上是存取`{主專案目錄}/public/upload/avatar.png`。

> **注意**
> 以`/app/xx/檔案名`開頭的靜態檔案存取實際上是存取應用程式插件的`public`目錄，也就是說不支援`{主專案目錄}/public/app/`下的目錄存取。
