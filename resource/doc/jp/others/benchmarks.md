...
なお、keep-aliveを無効にして圧測を行うと、クライアント側のローカルポートがTIME_WAIT状態の接続によってすぐに枯渇します。総リクエスト数がある閾値（通常約2.8万程度）を超えると、リクエストの失敗が発生することがあります。
...
**環境**
Alibaba Cloudの4コア4Gサーバー、ローカルMySQLデータベース、10万件のレコードからランダムに1件をクエリしてJSONで返す、ローカル圧測。
...
> **ヒント**
> ORMを使用するとパフォーマンスが低下する可能性がありますが、99%のビジネスシナリオでは既に性能が十分以上にあります。残りの1%に該当する場合でも、CPUやサーバーを追加することで簡単に対応できます。
> 開発効率、保守性、パフォーマンスなど、複数の基準に基づいてバランスを見極めるべきであり、パフォーマンスのみを盲目的に追求するべきではありません。
