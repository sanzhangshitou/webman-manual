...
在使用 `mongodb/laravel-mongodb` 之前必須先給 `php-cli` 安裝 mongodb 擴展。

> **注意**
> 使用命令 `php -m | grep mongodb` 查看 `php-cli` 是否裝了 mongodb 擴展。注意：即使你在 `php-fpm` 安裝了 mongodb 擴展，不代表你在 `php-cli` 可以使用它，因為 `php-cli` 和 `php-fpm` 是不同的應用程式，可能使用的是不同的 `php.ini` 配置。使用命令 `php --ini` 來查看你的 `php-cli` 使用的是哪個 `php.ini` 配置檔案。
...
## 配置
在 `config/database.php` 裏增加 `mongodb` connection，類似如下：
```php
return [
