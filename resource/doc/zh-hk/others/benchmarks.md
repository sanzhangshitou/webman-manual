...
## HTTP keep-alive是什麼？
HTTP Keep-Alive機制是一種用於在單個TCP連接上發送多個HTTP請求和響應的技術，它對於性能測試結果影響很大，關閉keep-alive後QPS可能成倍下降。目前瀏覽器都是默認開啟keep-alive的，也就是瀏覽器訪問某一個http地址後會將連接暫時保留不關閉，下一次請求時複用這個連接，用來提高性能。壓測時建議開啟 keep-alive。
另外，如果不開啟 keep-alive 壓測，客戶端本地埠很快會被 timewait 狀態耗光，表現為總請求數超過一定數量（一般約 2.8 萬左右），就會出現失敗的請求。

## 壓測時如何開啟 HTTP keep-alive？
...
**環境**
伺服器阿里雲 4 核 4G，本地 MySQL 資料庫，從 10 萬條記錄中隨機查詢一條資料 json 返回，本機壓測。
...
> **提示**
> 雖然使用 ORM 性能會有所下降，但是對於 99% 的業務來說性能已經嚴重溢出，如果你剛好是那 1% 也可以透過增加 CPU 或伺服器輕鬆解決。
> 我們應該在開發效率、可維護性、性能等多個指標中找到一個平衡點，而不是一味追求性能。
